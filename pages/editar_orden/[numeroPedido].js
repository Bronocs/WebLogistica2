...
export default function Editar_Orden() {
  const router = useRouter();
  const { numeroPedido } = router.query;
  const [rowIndicesOriginales, setRowIndicesOriginales] = useState([]);
  const [materiales, setMateriales] = useState([
    { cantidad: '', producto: '' }
  ]);
  useEffect(() => {
    if (!numeroPedido) return;
    fetch(`/api/encontrar_orden?nombrePedido=${encodeURIComponent(numeroPedido)}`)
      .then(res => res.json())
      .then(filas => {
        if (filas.length === 0) return;
        // Ahora accede a la columna correcta: por ejemplo, columna 1 es nombreProyecto
        setNombreProyecto(filas[0][1] || "");
        setRowIndicesOriginales(filas.map(row => row._rowIndex).filter(idx => idx));
        console.log(filas);

        // Mapear solo cantidad (índice 6) y producto (índice 4)
        const mats = filas.map(row => ({
          cantidad: row[6] || "",
          producto: row[4] || "",
          _rowIndex: row._rowIndex || "",
        }));

        setMateriales(mats.concat([{ cantidad: "", producto: "" }]));
      })
      .catch(err => {
        console.error("Error al leer datos del pedido:", err);
      });
  }, [numeroPedido]);
...
const handleInputChange = (idx, campo, valor) => {
  ...
  const enviarPedidos = async () => {
    ...
    const rowIndicesActuales = materialesFinal
      .map(m => m._rowIndex)
      .filter(idx => idx);

    const eliminados = rowIndicesOriginales.filter(idx => !rowIndicesActuales.includes(idx));

    setLoading(true);
    try {
      const res = await fetch('/api/guardar_editado', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setExito(data.numeroPedido); // activa el modal de éxito
                ...
                <React.Fragment key={idx}>
                  ...
                  {(productosInvalidos.includes(idx) || cantidadesInvalidas.includes(idx)) && 
                   (idx !== materiales.length - 1 || keyEnviado) && (
                    <tr>
                      {/* Solo error de producto -> empieza en columna de productos */}
                      {productosInvalidos.includes(idx) && !cantidadesInvalidas.includes(idx) && (
                        <>
                          <td style={{ padding: '4px 8px' }}></td>
                          <td style={{ padding: '4px 8px' }}>
                            <div style={{ color: 'red', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '4px', justifyContent: 'center' }}>
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width="14" height="14" fill="currentColor">
                                <path d="M320 64C334.7 64 348.2 72.1 355.2 85L571.2 485C577.9 497.4 577.6 512.4 570.4 524.5C563.2 536.6 550.1 544 536 544L104 544C89.9 544 76.9 536.6 69.6 524.5C62.3 512.4 62.1 497.4 68.8 485L284.8 85C291.8 72.1 305.3 64 320 64zM320 232C306.7 232 296 242.7 296 256L296 368C296 381.3 306.7 392 320 392C333.3 392 344 381.3 344 368L344 256C344 242.7 333.3 232 320 232zM346.7 448C347.3 438.1 342.4 428.7 333.9 423.5C325.4 418.4 314.7 418.4 306.2 423.5C297.7 428.7 292.8 438.1 293.4 448C292.8 457.9 297.7 467.3 306.2 472.5C314.7 477.6 325.4 477.6 333.9 472.5C342.4 467.3 347.3 457.9 346.7 448z"/>
                              </svg>
                              Falta producto
                      ...
                      {/* Solo error de cantidad -> empieza en columna de cantidad */}
                      {cantidadesInvalidas.includes(idx) && !productosInvalidos.includes(idx) && (
                        <>
                          <td style={{ padding: '4px 8px' }}>
                            <div style={{ color: 'red', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width="14" height="14" fill="currentColor">
                                <path d="M320 64C334.7 64 348.2 72.1 355.2 85L571.2 485C577.9 497.4 577.6 512.4 570.4 524.5C563.2 536.6 550.1 544 536 544L104 544C89.9 544 76.9 536.6 69.6 524.5C62.3 512.4 62.1 497.4 68.8 485L284.8 85C291.8 72.1 305.3 64 320 64zM320 232C306.7 232 296 242.7 296 256L296 368C296 381.3 306.7 392 320 392C333.3 392 344 381.3 344 368L344 256C344 242.7 333.3 232 320 232zM346.7 448C347.3 438.1 342.4 428.7 333.9 423.5C325.4 418.4 314.7 418.4 306.2 423.5C297.7 428.7 292.8 438.1 293.4 448C292.8 457.9 297.7 467.3 306.2 472.5C314.7 477.6 325.4 477.6 333.9 472.5C342.4 467.3 347.3 457.9 346.7 448z"/>
                              </svg>
                      ...
                      {/* Ambos errores -> empieza en columna de cantidad */}
                      {cantidadesInvalidas.includes(idx) && productosInvalidos.includes(idx) && (
